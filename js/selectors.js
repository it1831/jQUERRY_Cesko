...
$(function () {
    ...
    */
     ...
     */
    $(".btn-danger").on("click", () => {
        $("h1:first-child").css({ "background-color": "black" }).text("A máte k tomu účtenku?");
        $("h2, h3").css({ "background-color": "yellow", "color": "gray" }).text("Sorry JAKO");
        $("th").css({ "background-color": "red" }).text("silné")
        $("td").css({ "background-color": "red" }).html("<b>česko</b>")
        $("nav").hide(500);
        $("figcaption").hide(500);

        $("img").fadeOut(800, function () {
            $(this).attr("src", "img/babis_1.jpg").fadeIn(500);
        });
        $("[alt]").css({ "border": "10px solid red", "padding": "5px" });
        $("p:even").fadeOut(500, function () {
            $(this).html("Představte si, že bych byl Harry Potter, měl kouzelnou hůlku a proměnil ty projekty hned teď ve skutečnost. Stali bychom se okamžitě druhým Švýcarskem<br>Představte si, že bych byl Harry Potter, měl kouzelnou hůlku a proměnil ty projekty hned teď ve skutečnost. Stali bychom se okamžitě druhým Švýcarskem<br>Představte si, že bych byl Harry Potter, měl kouzelnou hůlku a proměnil ty projekty hned teď ve skutečnost.Stali bychom se okamžitě druhým Švýcarskem").fadeIn(500);
        });
        $("p:odd").fadeOut(500, function () {
            $(this).text("Představte si, že bych byl Harry Potter, měl kouzelnou hůlku a proměnil ty projekty hned teď ve skutečnost. Stali bychom se okamžitě druhým Švýcarskem").fadeIn(500);
        });
